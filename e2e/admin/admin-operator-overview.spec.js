...
test.describe('Admin Operator Overview — HOG stats match DB', () => {
  ...
  test('active_members stat card displays correct value', async ({ page, context }) => {
    await auth.setAdminCookieOnContext(context);
    await page.goto(`/operator/${seed.HOG_CLIENT_ID}`);
    await page.waitForLoadState('networkidle');
    const content = await page.content();
    expect(content).toContain(String(stats.active_members));
  });
...
test.describe('Admin Operator Overview — Location cards', () => {
  ...
  test('Test location appears on Test client overview', async ({ page, context }) => {
    await auth.setAdminCookieOnContext(context);
    await page.goto(`/operator/${seed.TEST_CLIENT_ID}`);
    await page.waitForLoadState('networkidle');
    const content = await page.content();
    expect(
      content.includes('E2E Test') || content.includes(seed.TEST_LOCATION_ID)
    ).toBe(true);
  });
