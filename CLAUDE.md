...
| Table | Purpose |
|---|---|
| `connector_subscriptions` | Per-(client + hardware platform). `hardware_platform` string + `hardware_api_key` (encrypted) + `kisi_user_pattern` (`'invited'` default, CHECK invited/managed — DR-043; moved here from `clients` in the S-10 cutover 2026-05-12, read by `standard-adapter.js#resolveIdentity` and `plan-mapping-resolver.js`). Canonical post-cutover; supersedes `clients.hardware_platform`. |
...
| DR-043 | Per-tenant Kisi user pattern (`kisi_user_pattern VARCHAR(20) DEFAULT 'invited'` — originally on `clients`, moved to `connector_subscriptions` in the S-10 cutover 2026-05-12). Supersedes DR-017. |
